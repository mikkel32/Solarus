...
def _strip_notebook_launcher_args(argv: Sequence[str]) -> List[str]:
    """Remove Jupyter/Colab kernel arguments that confuse ``argparse``.

    When scripts are executed via ``%run`` or assigned as a Colab cell entry point, the
    underlying kernel forwards a ``-f`` flag that points at a JSON connection file. The
    trainer does not recognise this option, so we quietly discard it (and its value)
    before parsing arguments. All other flags are preserved verbatim.
    """
...
def main() -> None:
    ...
    args = parser.parse_args(_strip_notebook_launcher_args(sys.argv[1:]))
